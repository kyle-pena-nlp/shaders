// https://www.dropbox.com/s/403xqls3yka22b4/jolly_santa_triptogram.png?dl=0

#define PENGUIN 2

#define PENGUIN_SCALE 0.1
...
// { "zoomout": ("-0.1", 0.25), "nakedeye": ("-1.",0.65), "wayout": ("-10.", 0.1)}
#define CONSTANT_REAL_TERM -10.

// { "polar": ("1",0.25), "cartesian": ("0",0.75) }
#define POLAR 0

// {  "regularness": ("0", 0.625), "velvet": ("1", 0.125), "neons": ("2", 0.125), "fantasy": ("3", 0.125) } 
#define SHADE_STYLE 1
...
Result newtown_raphson(Z z, Poly3 poly, Z multiplier) {
    ...
    vec2 uv = vec2(z.real,z.imag);

    for (int i = 0; i < MAX_ITER; i++) {
        float d = z.real*z.real + z.imag*z.imag;
        
        if (i == 4) {
            uv = vec2(z.real, z.imag);
        }
    ...
    return Result(MAX_ITER, z, 0., tot_d, uv);
}
...
vec3 render(in vec2 fragCoord, float time ) {
    ...
    #else
        float shade = vec2(0.); // deliberate compile error
    #endif
    
    #if PENGUIN == 1
    root_color = mix(texture(iChannel0, PENGUIN_SCALE * result.uv).xyz, root_color, shade > 0.5 ? 0.25 : 1.);
    shade = shade > 0.5 ? shade : 1.0;
    //shade = 1.0;
    #elif PENGUIN == 2
    root_color = texture(iChannel0, 0.15 * result.uv.yx).xyz;
    #elif PENGUIN == 3
    root_color = mix(texture(iChannel0, PENGUIN_SCALE * result.uv).xyz, root_color, shade > 0.5 ? 0.5 : 0.);
    shade = shade > 0.5 ? shade : 1.0;    
    #endif
